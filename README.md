...
Методология вводит правила разбиения верстки на независимые блоки, выделения элементов внутри блоков, а также правила повторного использования блоков с заданными модификациями поведения или внешнего вида. Подробнее почитать о ней можно [тут](https://ru.bem.info/method/).
...
Инициализация компонентов происходит так. При отрисовке html страницы после загрузки кода i-bem и готовности DOM i-bem анализирует декларации блоков страницы (пример декларации будет ниже). i-bem ищет на странице DOM-узлы с классами, аналогичными названиям декларированных блоков. К примеру, пусть имеется декларация
```javascript
BEM.DOM.decl('my-block');
```
,тогда i-bem, найдя DOM-узлы с классом `i-bem, отберет из них те, у которых в классах также присутствует класс `my-block`. Затем у каждого такого DOM-узла проверит наличие в атрибуте `data-bem` JSON объекта со свойством 'my-block'. Если все проверки прошли успешно, значит этому DOM-узлу соответствует i-bem блок, и он будет проинициализирован.
...
Это шаблонизаторы, генерирующие html код, написанный по правилам БЭМ методологии. Принимают на вход объекты в особом формате — написанном в соответствии с соглашением BEMJSON. BEMJSON-файл похож на JSON-файл. Вот пример.
...
Borschick выполняет функции постпроцессора (аналог Rework, PostCSS). Позволяет производить пост-обработку стилей и скриптов. С его помощью можно настроить объединение картинок в спрайты, минификацию, замену переменных и т.д. В примере мы не будем использовать этот инструмент, т.к. эти операции стоит делегировать более распространенным инструментам.
